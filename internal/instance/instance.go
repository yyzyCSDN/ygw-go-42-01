...
func (m *Manager) MarkHealthy(instanceID string) bool {
	// BUG(06b): health promotion is applied from whatever state the
	// instance currently holds; a stale retry that already overwrote the
	// registration is then promoted as healthy, freezing the rollback in
	// place so the registry keeps serving the reverted endpoint.
