...
func (m *Manager) MarkSuspect(instanceID string) bool {
	// BUG(10b): lifecycle transitions operate on whatever instance is in
	// the registry, so a resurrected instance can be promoted back to
	// healthy and the delete is permanently overwritten.
	inst, ok := m.reg.Get(instanceID)
	if !ok {
		return false
	}
