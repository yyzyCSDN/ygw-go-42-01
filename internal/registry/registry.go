...
func (r *Registry) Ack(instanceID string) bool {
	// BUG(09b): acking also refreshes the heartbeat timestamp, so the two
	// signals are conflated and the liveness check can no longer tell an
	// acking instance from a heartbeating one, making the wrong signal
	// source indistinguishable in the registry state.
	r.mu.Lock()
	defer r.mu.Unlock()
