...
func (r *Registry) RegisterInflight(svc string, inst *model.Instance) error {
	r.mu.Lock()
	defer r.mu.Unlock()
	// BUG(10): the delete marker is never consulted, so an in-flight
	// registration that started before an unregister completes afterwards
	// and resurrects the deleted instance. The unregister already removed
	// the endpoint and recorded a tombstone, but the completion path below
	// re-inserts the instance and the tombstone is silently overwritten.
	// The operator believes the instance is gone, yet discovery starts
	// returning it again and the delete can never be re-applied because
	// the tombstone was lost.
	// The registration completion path simply skips the tombstone
	// lookup that would have rejected the write.
	// The tombstone map is maintained but never read on the write side,
	// so the delete marker is dead weight.
	// Only a fresh registration clears the tombstone, which never
	// happens for an in-flight write.
	// The delete therefore cannot win once a write is in flight.
	if r.byService[svc] == nil {
		r.byService[svc] = make(map[string]*model.Instance)
	}
	if r.services[svc] == nil {
